...
void Score::init()
{
    ...
    if (status->getNbDeaths() == 0 && status->getMaxLife() == 20 && status->getMaxMagic() == 32 && !inventory->hasObject(FLACON_1) && !inventory->hasObject(FLACON_2) && !inventory->hasObject(FLACON_3) && !inventory->hasMedaillons(0) && !inventory->hasMedaillons(1) && !inventory->hasMedaillons(2) && !link->getBouclier())
    {
