...
int TransitionMaps::getSecondMapId(int x, int y) {
    switch (map1->getId()) {
        case 1 :
            if (direction == W && y / 240 < 1) return 6;
            if (direction == W && y / 240 >= 1) return 2;
            if (direction == S) return 8;
            break;
        case 2 :
            if (direction == N) return 6;
            if (direction == S) return 3;
            if (direction == W) return 4;
            if (direction == E && y / 240 < 2) return 1;
            if (direction == E && y / 240 >= 2) return 8;
            break;
        case 3 :
            if (direction == N && x / 320 < 2) return 4;
            if (direction == N && x / 320 >= 2) return 2;
            if (direction == W) return 9;
            if (direction == E) return 8;
            break;
        case 4 :
            if (direction == N) return 7;
            if (direction == S) return 3;
            if (direction == W) return 5;
            if (direction == E) return 2;
            break;
        case 5 :
            if (direction == N) return 7;
            if (direction == S) return 9;
        ...
        case 6 :
            ...
            break;
        case 7 :
            if (direction == S && x / 320 < 5) return 5;
            if (direction == S && x / 320 >= 5) return 4;
            if (direction == E) return 6;
            break;
        case 8 :
            if (direction == N) return 1;
            if (direction == W && y / 240 < 2) return 2;
            if (direction == W && y / 240 >= 2) return 3;
            break;
        case 9 :
