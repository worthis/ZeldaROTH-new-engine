...
int Link::getDefense() {
    return getBouclier()
            + (inventory->hasMedaillons(0) ? 1 : 0)
            + (inventory->hasMedaillons(1) ? 1 : 0)
            + (inventory->hasMedaillons(2) ? 2 : 0);
}
...
void Link::handleAction(Action* action) {
    ...
    if (porte != 0 && animation != SOULEVE && ((action->isAction(HIT) && epee != 0)
                                               || action->isAction(ACTION) || action->isAction(OBJECT))) {
...
void Link::trouveObjet(TypeItem type, int id) {
    ...
    if ((animation == IDLE || animation == WALK || animation == PUSH) && charge == 0 && !lapin) {
        ...
        if (type == TI_QUART_COEUR || type == TI_EPEE_1 || type == TI_EPEE_3 || type == TI_EPEE_5
             || type == TI_CLE || type == TI_CLE_BOSS || type == TI_CARTE_DONJON || type == TI_BOUSSOLE
             || type == TI_MEDAILLON_1 || type == TI_MEDAILLON_2 || type == TI_MEDAILLON_3) {
