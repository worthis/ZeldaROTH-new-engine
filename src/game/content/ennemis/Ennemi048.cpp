...
void Ennemi048::giveItem(int x, int y) {
    ...
    if (scene->getMap()->getId() == 20 && bounds->getX() == 320 * 10 && bounds->getY() == 240 * 2 && !scene->getCoffre(9, 7)) {
        AudioManager::getInstance()->playSound(TS_KILLENNEMY);
        Map* map = MainController::getInstance()->getGameController()->getSceneController()->getScene()->getMap();
        map->addEffect(new FumeeBlanche(x, y));
        scene->getMap()->addItem(ItemHelper::getInstance()->createItem(TI_CLE, x, y, 7, true));
    } else {
        Ennemi::giveItem(x, y);
    }
}
